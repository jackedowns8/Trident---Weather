...
function add_wm_wxtiles(dt){
    ...
    wxoverlay = new WXTiles({withnone:true,autoupdate:false,cview:localStorage.getItem("mapView"),updateCallback:loadTimes});
    map.id='charts-map';
    //add new satellite and weather objects to map
    wxoverlay.addToMap(map);
    //add legend color bar
    wxoverlay.addColorBar('small', 'horiz', 'BottomLeft');
    map.setZoom(4);
    return map;
}
